# Rust Slot Battle 0.1.12 — スクラップ方式
...
- v0.1.9：実機ボタンとクリック領域を再調整。透明リール更新パネルのクリック遮断を除去。BB中の通常背景を対応する戦闘画像へ固定。
- v0.1.10：科学者4色を背景透過オーバーレイ化。現在のステージ／BB戦闘背景を保ったまま、巨大な影→接近→色判明→実体を重ねて表示。
- v0.1.11：リールを約16FPSの縦スクロール表示へ変更。停止したリールだけ確定位置へ吸着し、残りのリールは滑らかに回転を継続。
- v0.1.12：リール窓マスクのステンシル透明度を修正し、回転記号が黒画面になる問題を解消。
...
## 画像と4コマ演出
青・黄・緑・赤科学者は、背景透過PNGとして `assets/overlays/` に収録。従来の固定背景版は互換・バックアップ用として `assets/animations/` に残す。
