# Rust Slot Battle 0.1.8 — スクラップ方式
...
## 実装範囲
- 正式採用の錆びた筐体フレーム、独立データカウンター、大型上部画面、3リール×21コマ（独自配列）、停止ボタン。
- ステージ4種、BB戦闘15種、強前兆、科学者4色の画像を自動登録。演出終了後も通常画面へ切れ目なく復帰。
...
## 0.1.4の追加
- START、リール停止、押し順ナビ、科学者出現、小役払出、BB開始／継続、敗北へ効果音を追加。
- 効果音は操作したプレイヤー本人の接続だけへ送信し、他プレイヤーのスロット音と混ざらない。
- `oxide/config/RustSlots.json` の `EnableSounds` で一括ON/OFF可能。
- 各Sound項目のeffect prefabパスを変更すれば、演出音を個別に差し替え可能。

## 0.1.5の強化
- 地味だった単発通知音を短い音列へ変更。
- 押し順ナビは警告音を含む5連へ強化し、上部画面を黄色に変えて停止順を中央へ大きく表示。
- 青・黄・緑・赤の科学者は色ごとに異なる音列で出現。赤ほど強い警告になる。
- 強前兆の影には専用の不穏な4連音を追加。払出は2段、BB確定／継続は4段、敗北は3段で再生。
- `SoundSequenceGap` で音同士の間隔を0.06～0.5秒に調整可能。既定値は0.12秒。
- 音列もプレイヤー本人の接続だけへ送信する。

## 0.1.6の追加
- 上部画面左側へ実機風の `DATA COUNTER` を追加。
- 総ゲーム、BB回数、現在ゲーム、最高ROUND、累計払出SCをプレイヤー別に永続保存。
